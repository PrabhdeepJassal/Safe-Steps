...
- **Authentication**: Complete login/signup flow with email and phone number support, password visibility toggle, and optional skip functionality.
- **Navigation**: Stack-based navigation between authentication screens and main application.

**Database**
- **Type**: MongoDB for storing user profiles, preferences, and processed crime data.
- **Authentication**: Node.js backend with Express.js and Mongoose ODM for MongoDB operations.
- **Schema**:
  - **User Data**: 
    - Name, email (optional), phone (optional), encrypted password
    - Additional fields: age, blood type, medical conditions
    - Separate email and phone fields supporting both login methods
  - **Crime Data**: Latitude, Longitude, Severity, CrimeID, CrimeCategory.
- **Security**: Password hashing with bcrypt, unique constraints on email and phone fields.
...
- **Backend Authentication**: Custom Node.js authentication system with MongoDB:
  - Email and phone number registration/login support
  - bcrypt password hashing
  - Input validation and error handling
  - Automatic detection of email vs phone number format
...
- **Backend**:
  - **ML Service**: Python 3.9+ with Flask 2.3+, Flask-CORS, Pandas, NumPy, Scikit-learn, SciPy, Geopy.
  - **Authentication Service**: Node.js with Express.js, MongoDB, Mongoose, bcrypt, cors.
...
2. **Backend Setup**:
   - **Authentication Service**:
     ...
     - Create a `.env` file in the backend directory:
       ...
       PORT=5001
       ```
     - Start the Node.js authentication server:
       ```sh
       npm start
       ```
   - **ML Service** (if using Python backend for routes):
     ```sh
     cd ml-backend  # or appropriate directory
...
3. **Frontend Setup**:
   - Install dependencies:
     ```sh
     cd Safe-Steps  # or frontend directory
     npm install
     ```
   - Create a `.env` file in the frontend directory:
     ```sh
     GOOGLE_MAPS_API_KEY=your_google_maps_api_key
     API_BASE_URL=http://192.168.29.111:5001  # Authentication API
     ML_API_BASE_URL=http://localhost:5000     # ML Service API (if separate)
     ```
   - Update the API base URL in `utils/api.js` to match your backend server IP.
   - Run the app:
     ```sh
...
4. **Database Setup**:
   ...
   - The authentication system will automatically create the required collections:
     - `users` - for user authentication and profile data
   - Initialize collections for crime data (handled automatically by ML backend).
   - Test the authentication endpoints:
     ```sh
     # Test signup
     curl -X POST http://localhost:5001/api/auth/signup \
       -H "Content-Type: application/json" \
       -d '{"name":"Test User","emailPhone":"test@example.com","password":"password123"}'
     
     # Test login
     curl -X POST http://localhost:5001/api/auth/login \
       -H "Content-Type: application/json" \
       -d '{"emailPhone":"test@example.com","password":"password123"}'
     ```
...
1. **User Authentication**: 
   - Users can register with email or phone number via the signup screen.
   - Login supports both email and phone number with automatic format detection.
   - Optional "Skip" functionality allows exploring the app without account creation.
   - Passwords are securely hashed using bcrypt before storage.
2. **User Input**: The user enters a source, destination, and optional time category (Morning, Afternoon, Evening, Night) via the React Native app.
3. **Route Generation**: The backend queries OSRM to fetch 6–7 routes, including direct and waypoint-based alternatives.
4. **Crime Analysis**:
...
6. **Route Ranking**: Sorts routes by safety score (highest first) and returns them to the frontend.
7. **Visualization**: The frontend displays routes on a map, with the safest route highlighted and crime hotspots marked.
8. **Emergency Features**: Users can trigger alerts or enable periodic check-ins during travel.
